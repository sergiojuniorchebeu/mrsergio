...
// Spotlight Glow Card — cursor-reactive border, image hover overlay
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function getPreviewImage(p: Project) {
    return p.screenshots_urls?.[0] ?? p.image_url ?? null;
}

function getPrimaryLink(p: Project) {
...
function getCtaLabel(p: Project) {
    switch (p.project_type) {
        case 'mobile':  return "Voir l'app";
        case 'desktop': return 'Voir le logiciel';
        case 'api':     return "Voir l'API";
        default:        return 'Voir le projet';
    }
}
    ...
    return (
        ...
        >
            <SpotlightCard className="h-full">
                {/* Full-card nav (z-10, below hover actions z-20) */}
                <Link
                ...
                {/* ── Image ────────────────────────────────────────────── */}
                <div className="relative h-48 sm:h-52 overflow-hidden bg-gradient-to-br from-slate-50 to-teal-50/20 flex-shrink-0">
                    {previewImage ? (
                        <img
                            src={previewImage}
                            alt={project.title}
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                            <div className="flex flex-col items-center gap-3 text-teal-300">
                                ...
                                <span className="text-xs font-medium text-slate-400">{project.title}</span>
                            </div>
                        </div>
                    )}

                    {/* Gradient overlay on hover */}
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-900/65 via-slate-900/20 to-transparent opacity-0 transition-opacity duration-350 group-hover:opacity-100" />

                    {/* Hover action buttons z-20 */}
                    <div className="absolute inset-0 z-20 flex items-center justify-center gap-3 opacity-0 translate-y-3 transition-all duration-300 group-hover:opacity-100 group-hover:translate-y-0">
                        {project.github_url && !project.private_repo && (
                            <a
                ...
                <div className="flex flex-col flex-1 p-5 gap-3">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        ...
                        {project.tags.length > 3 && (
                            <span className="text-[11px] px-2.5 py-0.5 rounded-full border bg-slate-50 text-slate-400 border-slate-100">
                                +{project.tags.length - 3}
                            </span>
                        )}
                    </div>
                    ...
                    {/* Title */}
                    <h3 className="font-display font-semibold text-[16px] leading-snug text-slate-900 group-hover:text-teal-600 transition-colors duration-300">
                        {project.title}
                    </h3>
                    ...
                    <div className="mt-1 pt-4 border-t border-slate-100 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1.5">
                            {project.github_url && (
                                <span title={project.private_repo ? 'Repo privé' : 'GitHub'} className={cn('flex h-6 w-6 items-center justify-center rounded-lg', project.private_repo ? 'bg-slate-100 text-slate-300' : 'bg-slate-100 text-slate-500')}>
                                    <GithubIcon className="w-3.5 h-3.5" />
                                </span>
                            )}
                            {externalLink && (
                                <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-teal-50 text-teal-500">
