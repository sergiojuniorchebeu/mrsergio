...
export default function FormationForm({ formation }: { formation: Formation | null }) {
    ...
    return (
        <AdminLayout title={isEdit ? 'Modifier la formation' : 'Nouvelle formation'}>
            <Head title={`${isEdit ? 'Modifier' : 'Créer'} une formation — Admin`} />

            <div className="flex items-center gap-3 mb-6">
                <Link href="/admin/formations" className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-all">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
                </Link>
                <div>
                    <h2 className="text-xl font-display font-bold text-slate-800">{isEdit ? 'Modifier la formation' : 'Nouvelle formation'}</h2>
                    {isEdit && <p className="text-xs text-slate-400 mt-0.5">ID #{formation.id}</p>}
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-5">
                        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                            ...
                            <Field label="Description courte" required error={errors.excerpt}>
                                <textarea value={values.excerpt} onChange={e => set('excerpt', e.target.value)}
                                    placeholder="Résumé de la formation..." rows={3}
                                    className={cn(inputCls, 'resize-none', errors.excerpt && errorInputCls)} />
                            </Field>
                            <Field label="Programme (Markdown)" error={errors.content}>
                        ...
                        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                            ...
                            <h3 className="text-sm font-semibold text-slate-700">Détails pédagogiques</h3>
                            <div className="grid sm:grid-cols-2 gap-4">
                                <Field label="Catégorie" required error={errors.category}>
                                    <select value={values.category} onChange={e => set('category', e.target.value)} className={cn(inputCls, errors.category && errorInputCls)}>
                                        {categories.map(c => <option key={c}>{c}</option>)}
                                    </select>
                                </Field>
                                <Field label="Niveau" required error={errors.level}>
                                    <select value={values.level} onChange={e => set('level', e.target.value)} className={cn(inputCls, errors.level && errorInputCls)}>
                                        {levels.map(l => <option key={l}>{l}</option>)}
                                    </select>
                                </Field>
                                <Field label="Langue" error={errors.language}>
                                    <input type="text" value={values.language} onChange={e => set('language', e.target.value)} placeholder="Français" className={inputCls} />
                                </Field>
                                <Field label="Durée (heures)" required error={errors.duration_hours}>
                                    <input type="number" value={values.duration_hours}
                                        onChange={e => set('duration_hours', parseInt(e.target.value) || 1)}
                                        min={1} className={cn(inputCls, errors.duration_hours && errorInputCls)} />
                                </Field>
                                <Field label="Nombre de leçons" required error={errors.lessons_count}>
                                    <input type="number" value={values.lessons_count}
                                        onChange={e => set('lessons_count', parseInt(e.target.value) || 1)}
                                        min={1} className={cn(inputCls, errors.lessons_count && errorInputCls)} />
                                </Field>
                            </div>
                        </motion.div>
                    ...
                    <div className="space-y-5">
                        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                            ...
                            className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-5 space-y-4">
                            <h3 className="text-sm font-semibold text-slate-700">Prix</h3>
                            <label className="flex items-center justify-between cursor-pointer">
                                <span className="text-sm text-slate-700">Formation gratuite</span>
                                <button type="button" onClick={() => set('is_free', !values.is_free)}
                                    className={cn('relative w-10 h-5 rounded-full transition-colors duration-200', values.is_free ? 'bg-teal-500' : 'bg-slate-300')}>
                                    <span className={cn('absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-transform duration-200', values.is_free ? 'translate-x-5' : 'translate-x-0.5')} />
                                </button>
                            </label>
                            {!values.is_free && (
                        ...
                        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                            ...
                            <label className="flex items-center justify-between cursor-pointer">
                                <span className="text-sm text-slate-700">Publiée</span>
                            ...
                            <label className="flex items-center justify-between cursor-pointer">
                                <span className="text-sm text-slate-700">Mise en avant</span>
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <Link href="/admin/formations" className="w-full flex items-center justify-center py-3 rounded-xl border border-slate-200 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">Annuler</Link>
                        </div>
                    </div>
                </div>
            </form>
