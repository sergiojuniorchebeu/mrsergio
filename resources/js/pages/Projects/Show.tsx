...
import type { ScreenshotCard }      from '@/components/ui/screenshot-carousel'
...
export default function Show({ project, related }: ProjectShowProps) {
  ...
  // Transforme les URLs de screenshots en ScreenshotCard[]
  const carouselCards = useMemo<ScreenshotCard[]>(() =>
    (project.screenshots ?? []).map((src, i) => ({
      src,
      title:    project.title,
      category: `Capture ${i + 1} / ${project.screenshots?.length ?? 1}`,
    })),
    [project.screenshots, project.title],
  )
    ...
    <MainLayout>
      ...
      {carouselCards.length > 0 && (
        <section className="pb-16 sm:pb-20 overflow-hidden">
          ...
          >
            <ScreenshotCarousel cards={carouselCards} />
          </motion.div>
        </section>
      )}
